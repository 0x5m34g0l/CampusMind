...
namespace CampusMind.Logic.Services
{
    ...
    public static class AuthService
    {
        ...
        public static User Register(string email, string passwordPlain, string name)
        {
            ...
            int newID = Data.DataAccess.UserDataAccess.AddNewUser(email, passwordHash, name);
            if (newID != -1)
            {
        ...
        public static User Login(string email,string passwordPlain)
        {
            ...
            bool isFound = Data.DataAccess.UserDataAccess.GetUserByEmail(email, ref id, ref storedHash, ref name);
            if (!isFound)
            {
                return null;
            }
